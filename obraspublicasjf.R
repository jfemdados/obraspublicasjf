...
#Classificando tipo da obra
  ...
  mutate(class_objeto = case_when(
    ...
    str_detect(Objeto, "pavimentação|asf[aá]l|viaduto") ~ "infraestrutura de transportes",
    str_detect(Objeto, "contenção") ~ "contenção de morros",
    str_detect(Objeto, "despoluição") ~ "meio ambiente",
    str_detect(Objeto, "ginásio") ~ "ginasio",
    TRUE ~ "outros"
  ))%>%
...
valor_por_area %>%
  ...
  labs(title= "Valor das Obras da Prefeitura de Juiz de Fora por Área", caption =  "Fonte: Prefeitura JF" ) +
  theme_bw()
